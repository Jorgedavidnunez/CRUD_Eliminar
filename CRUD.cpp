...
void eliminar_estudiante (){
	...
	estudiante.codigo = -1;
	fwrite( &estudiante, sizeof(Estudiante), 1, archivo );
	fclose(archivo); //hasta aquí se cambia el codigo a -1
	...
	fclose(temporal);
	fclose(archivo);//pasa todo al temporal menos el -1
	
	archivo = fopen(nombe_archivo, "w");
	fclose(archivo); //vacia el archivo
	
	temporal  = fopen(nom_temporal,"r+b");
	
	fread ( &estudiante, sizeof(Estudiante), 1, temporal );
	...
	fclose(archivo);
	fclose(temporal); //pasa lo que hay en el temporal al archivo
	
	temporal = fopen(nom_temporal, "w");
	fclose(temporal); //vacia el temporal
